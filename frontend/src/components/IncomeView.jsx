...
const IncomeView = ({
  ...
  const avgIncomeVal = incomes.length > 0 ? totalIncomeVal / incomes.length : 0;
  const incomeCountVal = incomes.length;
  ...
  const aggregateData = () => {
    ...
    incomes.forEach((income) => {
      ...
      if (key) {
        dataMap[key] = (dataMap[key] || 0) + income.amount;
      }
    });
  ...
  const getChartTitle = () => {
    switch (selectedPeriod) {
      case 'daily':
        return 'Daily Income Trends';
      case 'weekly':
        return 'Weekly Income Trends';
      case 'monthly':
        return 'Monthly Income Trends';
      case 'yearly':
        return 'Yearly Income Trends';
      default:
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-left">
      ...
      <div className="lg:col-span-2 space-y-6">
        {/* Hero Card */}
        <div className="p-6 rounded-xl border border-[#e2e8f0] bg-white flex justify-between items-center shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div>
            <h2 className="text-xl font-bold text-[#334155] mb-1">Income Overview</h2>
            <p className="text-sm text-[#64748b]">Track and manage your income sources</p>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-[#10b981] hover:bg-[#0ea5e9] text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-all cursor-pointer shadow-sm"
          >
            <i className="fa-solid fa-plus text-xs"></i> Add Income
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-[#f8fafc] p-4 rounded-xl border border-[#e2e8f0]">
            <p className="text-xs text-[#64748b] flex items-center gap-1.5 mb-1">
              <i className="fa-solid fa-dollar-sign text-[#10b981]"></i> Total Income
            </p>
            <p className="text-xl font-bold text-[#334155]">${totalIncomeVal.toLocaleString()}</p>
          ...
          <div className="bg-[#f8fafc] p-4 rounded-xl border border-[#e2e8f0]">
            <p className="text-xs text-[#64748b] flex items-center gap-1.5 mb-1">
              <i className="fa-solid fa-chart-line text-[#6366f1]"></i> Average Income
            </p>
            <p className="text-xl font-bold text-[#334155]">
              ${avgIncomeVal.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </p>
            <p className="text-[10px] text-[#64748b] mt-1">
              <i className="fa-solid fa-list-check"></i> {incomeCountVal} transaction{incomeCountVal !== 1 ? 's' : ''}
            </p>
          </div>

          <div className="bg-[#f8fafc] p-4 rounded-xl border border-[#e2e8f0]">
            <p className="text-xs text-[#64748b] flex items-center gap-1.5 mb-1">
              <i className="fa-solid fa-arrow-trend-up text-[#a855f7]"></i> Transactions
            </p>
            <p className="text-xl font-bold text-[#334155]">{incomeCountVal}</p>
            <p className="text-[10px] text-[#64748b] mt-1">
        ...
        <div className="bg-white p-6 rounded-xl border border-[#e2e8f0] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-[#334155] flex items-center gap-2">
              <i className="fa-solid fa-chart-bar text-[#10b981]"></i> Income Analysis
            </h3>
            {/* Filter Buttons */}
        ...
        <div className="bg-white p-6 rounded-xl border border-[#e2e8f0] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-center mb-4 border-b border-[#f1f5f9] pb-3">
            <h3 className="font-semibold text-[#334155] flex items-center gap-2">
              <i className="fa-solid fa-dollar-sign text-[#10b981]"></i> Income Transactions
            </h3>
          ...
          {incomes.length === 0 ? (
            <div className="text-center py-12">
              <div className="w-16 h-16 bg-[#dcfce7] rounded-full flex items-center justify-center mx-auto mb-4">
                <i className="fa-solid fa-dollar-sign text-[#10b981] text-2xl"></i>
              </div>
              <h4 className="font-bold text-base text-[#334155] mb-1">No income transactions found</h4>
              <p className="text-sm text-[#64748b] mb-5">You haven't recorded any income yet</p>
              <button
                onClick={() => setIsModalOpen(true)}
                className="bg-[#10b981] text-white px-4 py-2 rounded-lg font-medium mx-auto flex items-center gap-2 hover:opacity-90 cursor-pointer"
              >
                <i className="fa-solid fa-plus text-xs"></i> Add Income
              </button>
            </div>
              ...
              {incomes.map((item) => (
                <li key={item._id} className="py-3.5 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#e6f9f7] text-[#00bfae] flex items-center justify-center text-sm font-semibold">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-[#e2e8f0] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
          <div className="flex justify-between items-center mb-4 border-b border-[#f1f5f9] pb-3">
            <h3 className="font-semibold text-[#334155] flex items-center gap-2">
              <i className="fa-solid fa-chart-pie text-[#10b981]"></i> Income by Category
            </h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-4">
            <div className="bg-[#f0f9ff] p-2.5 rounded-lg text-center">
              ...
              <p className="text-sm font-bold text-[#0ea5e9]">${totalIncomeVal.toLocaleString()}</p>
            </div>
            <div className="bg-[#fffbeb] p-2.5 rounded-lg text-center">
              ...
              <p className="text-sm font-bold text-[#f59e0b]">${(stats.totalExpenses || 0).toLocaleString()}</p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-[#0f172a]/40 backdrop-blur-[4px] flex items-center justify-center z-[1000] p-4 transition-opacity duration-300">
          <div className="bg-white w-full max-w-[440px] rounded-2xl p-7 shadow-2xl transform translate-y-0 transition-transform duration-300">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-lg font-bold text-[#1e293b]">Add New Income</h2>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4.5">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="incomeDescription" className="text-xs font-semibold text-[#334155]">
                  Description
                </label>
                <input
                  type="text"
                  ...
                  className="w-full px-3.5 py-2.5 border border-[#cbd5e1] rounded-lg text-sm text-[#334155] focus:border-[#10b981] focus:ring-3 focus:ring-[#10b981]/15 outline-none transition-all"
                  required
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label htmlFor="incomeAmount" className="text-xs font-semibold text-[#334155]">
                  Amount ($)
                </label>
                <input
                  type="number"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="incomeCategory" className="text-xs font-semibold text-[#334155]">
                  Category
                </label>
                <select
                  id="incomeCategory"
                  ...
                  onChange={handleInputChange}
                  className="w-full px-3.5 py-2.5 border border-[#cbd5e1] rounded-lg text-sm text-[#334155] bg-white focus:border-[#10b981] focus:ring-3 focus:ring-[#10b981]/15 outline-none transition-all"
                  required
                >
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="incomeDate" className="text-xs font-semibold text-[#334155]">
                  Date
                </label>
                <input
                  type="date"
              ...
              <button
                type="submit"
                className="w-full bg-[#10b981] hover:bg-[#00bfa5] text-white py-3 rounded-lg text-sm font-semibold cursor-pointer shadow-md transition-all active:scale-[0.98]"
              >
                Add Income
              </button>
            </form>
